...
September 11, 2026. Production promotion of the QA-tested support implementation; verify hosted readiness before switching the phone endpoint.

## Intended behavior

Escalate only when the caller requests human help or the agent cannot resolve an issue. Offer a phone connection/contact route or a message to support. Never email after every conversation. For email, collect the caller’s name, reply email and short issue summary, ask for spelling when uncertain, and read all three back, then obtain explicit approval before invoking `send_support_message` with `confirmed: true`.
...
Jason confirmed Tawk is unused. Its public and dashboard mounts, component, unused consent category and current privacy wording were removed in QA and are included in this production promotion. Preserve other tracking choices and existing saved analytics/marketing preferences. No Tawk account deletion is authorized or needed.


## Production release configuration

Enable SUPPORT_HANDOFF_ENABLED and configure SUPPORT_PHONE_HANDOFF_TOKEN on ia-backend-prod. Preserve its existing production SendGrid, Supabase, xAI keys and exact support origins. No schema migration is needed: the shared rate-limit RPC already exists in production.

The current public phone credential is moved to the production endpoint, then removed from QA by rotating the QA-only token after the phone URL has switched. Dashboard QA support continues using its session-authorized action. Neither agent receives a mail inbox connector. Do not send a production verification email without explicit user authorization.
