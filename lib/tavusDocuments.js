...
async function buildKbUrl(kbId, supabaseClient) {
  const key = normalizeKbKey(kbId);
  if (!key) return { error: missingTavusKbError(null, 'Invalid KB key') };
  const storage = supabaseClient.storage.from(KB_BUCKET);
  const { data: pubData } = storage.getPublicUrl(key);
  if (pubData?.publicUrl) return { url: pubData.publicUrl, key };
  const { data: signed, error } = await storage.createSignedUrl(key, 3600);
...
async function ensureTavusDocumentForRole(roleInput, opts = {}) {
  ...
  const kbUrl = await buildKbUrl(role.kb_document_id, supabaseClient);
  if (kbUrl?.error) {
